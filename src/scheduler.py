...
def run_system_bus_cycle():
    ...
    try:
        with db_transaction() as conn:
            # ?Phase 1: EVENT DETECTION (Sensors) ?            _detect_task_anomalies(connection=conn)
            _detect_human_interventions(connection=conn)
            _detect_stalled_activities(connection=conn)

            # ?Phase 2: EVENT CONSUMPTION (Control Plane) ?            _process_pending_events(connection=conn)

            # ?Phase 3: CONTROL GATE + DATA PLANE ?            state_data = execute_query("SELECT key, value FROM system_state", connection=conn)
            ...
            # ?Phase 4: RECONCILE ?            _reconcile_completed_tasks(connection=conn)
