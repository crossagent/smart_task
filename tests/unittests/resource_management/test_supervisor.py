...
def test_watchdog_restarts_dead_agent(mock_popen, supervisor):
    ...
    mock_proc_1.poll.return_value = None # 首个进程处于运行状�?    mock_popen.return_value = mock_proc_1
    ...
    # 2. 模拟进程死亡
    mock_proc_1.poll.return_value = 1 # 变为死亡状�?    assert not handle.is_alive()
    ...
    # 模拟 Watchdog 检查周�?    supervisor._reconcile_pool()
        
    # 验证 Popen 是否被调用了第二次（重启�?    assert mock_popen.call_count >= 2
    # 验证 handle 已绑定新进程
