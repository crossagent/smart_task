...
#   2. 过滤结果对象为 "database"。
#   3. 从搜索结果中匹配标题（例如 "任务" 或 "Tasks"）。
#   4. 确认 properties 中包含预期的字段（例如 "Task name", "Due"）。
#
# 注意：浏览器 URL 中的 ID 可能是页面 ID 或视图 ID，不一定能直接用于数据库操作。
# 如果 POST 请求返回 400 "is a page, not a database"，说明使用的是页面 ID。
#
# 最终确认的正确任务 Database ID：
#   1990d59d-ebb7-816d-ab7b-f83e93458d30
# ============================================================================

TASK_DB_ID = "1990d59d-ebb7-816d-ab7b-f83e93458d30"
...
def test_task_db_read_write_delete():
  """验证 Task DB 的写入→读取→归档删除完整流程。"""
      ...
      async with ClientSession(read, write) as session:
        ...
        create_res = await session.call_tool(
            "API-post-page",
            {
                ...
                "properties": {
                    "Task name": {
                        "title": [{"text": {"content": "[集成测试] Task 读写测试"}}]
                    },
        ...
        page_id = data["id"]
        print(f"\n[Test] Created task page: {data.get('url')}")
        ...
        print(f"[Test] Archived task page: {page_id}")
