...
#   2. 过滤结果对象为 "database"。
#   3. 从搜索结果中匹配标题（例如 "备忘录"）。
#   4. 确认 properties 中包含 "State" (status) 和 "Note" (title) 等字段。
#
...
MEMO_DB_ID = "3120d59d-ebb7-808d-a582-d4baae4fe44b"
...
def test_memo_db_read_write_delete():
  """验证 Memo DB 的写入→读取→归档删除完整流程。"""
      ...
      async with ClientSession(read, write) as session:
        ...
        create_res = await session.call_tool(
            "API-post-page",
            {
                ...
                "properties": {
                    "Note": {
                        "title": [{"text": {"content": "[集成测试] Memo 读写测试"}}]
                    },
                    "State": {"select": {"name": "未处理"}},
                },
            },
        )
        ...
        page_id = data["id"]
        print(f"\n[Test] Created memo page: {data.get('url')}")
        ...
        print(f"[Test] Archived memo page: {page_id}")
